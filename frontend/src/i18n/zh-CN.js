export default {
  app: {
    title: 'OpenCode Desktop',
    connected: '已连接',
    connecting: '连接中...',
    disconnected: '未连接',
  },
  common: {
    loading: '加载中',
    save: '保存',
    cancel: '取消',
    edit: '编辑',
    delete: '删除',
    close: '关闭',
    confirm: '确认',
  },
  contextMenu: {
    new: '新建',
    newFile: '文件',
    newFolder: '文件夹',
    copy: '复制',
    cut: '剪切',
    paste: '粘贴',
    rename: '重命名',
    delete: '删除',
    copyPath: '复制路径',
    openInFinder: '在访达中显示',
    newFileName: '文件名',
    newFolderName: '文件夹名',
    // Java
    javaClass: 'Java 类',
    javaInterface: 'Java 接口',
    javaEnum: 'Java 枚举',
    javaRecord: 'Java Record',
    javaAnnotation: 'Java 注解',
    javaPackage: 'Java 包',
    // Go
    goFile: 'Go 文件',
    goTest: 'Go 测试文件',
    // Python
    pyFile: 'Python 文件',
    pyPackage: 'Python 包',
    pyTest: 'Python 测试',
    // Vue/React
    vueComponent: 'Vue 组件',
    vueComposable: 'Vue Composable',
    reactComponent: 'React 组件',
    reactHook: 'React Hook',
    tsFile: 'TypeScript 文件',
    jsFile: 'JavaScript 文件',
    // Rust
    rsFile: 'Rust 文件',
    rsMod: 'Rust 模块',
    // C#
    csClass: 'C# 类',
    csInterface: 'C# 接口',
    csEnum: 'C# 枚举',
  },
  sidebar: {
    explorer: '资源管理器',
    search: '搜索',
    git: '源代码管理',
    settings: '设置',
    noFolder: '未打开文件夹',
    emptyFolder: '文件夹为空',
  },
  chat: {
    newSession: '新会话',
    sessions: '会话列表',
    new: '新建',
    noSessions: '暂无会话',
    placeholder: '输入问题或描述任务...',
    thinking: '思考中...',
    howCanIHelp: '有什么可以帮您？',
    askAnything: '问我任何关于编程的问题',
    you: '你',
    assistant: 'Kiro',
    cancel: '取消',
  },
  model: {
    select: '选择模型',
    free: '免费',
    premium: '付费',
    freeModels: '免费模型',
    premiumModels: '付费模型',
    customModels: '自定义模型',
  },
  terminal: {
    title: '终端',
    new: '新建终端',
  },
  panel: {
    problems: '问题',
    output: '输出',
    noProblems: '没有检测到问题',
    noOutput: '暂无输出',
  },
  editor: {
    openFile: '打开文件开始编辑',
    openFolder: '打开目录',
    showTerminal: '显示终端',
    hideTerminal: '隐藏终端',
    close: '关闭',
    closeLeft: '关闭左侧',
    closeRight: '关闭右侧',
    closeOthers: '关闭其他',
    closeAll: '关闭全部',
    // 右键菜单
    cut: '剪切',
    copy: '复制',
    paste: '粘贴',
    delete: '删除',
    selectAll: '全选',
    undo: '撤销',
    redo: '重做',
    find: '查找',
    replace: '替换',
    goToLine: '转到行',
    format: '格式化文档',
    comment: '切换行注释',
  },
  settings: {
    title: '设置',
    language: '语言',
    theme: '主题',
    general: '通用',
    models: {
      title: '模型管理',
      custom: '自定义模型',
      builtin: '内置模型',
      noCustom: '暂无自定义模型',
      add: '添加模型',
      edit: '编辑模型',
      modelId: '模型 ID',
      modelIdPlaceholder: '例如: gpt-4o',
      modelName: '显示名称',
      modelNamePlaceholder: '例如: GPT-4o',
      baseUrl: 'API 地址',
      baseUrlPlaceholder: '例如: https://api.openai.com/v1',
      apiKey: 'API Key',
      apiKeyPlaceholder: '输入 API Key',
      isFree: '免费模型',
      supportsImage: '支持图像输入',
      confirmDelete: '确定要删除模型 {name} 吗？',
    },
    mcp: {
      installed: '已安装',
      market: 'MCP 市场',
      noInstalled: '暂无已安装的 MCP 服务器',
      addManual: '手动添加',
      editFile: '编辑配置文件',
      addServer: '添加 MCP 服务器',
      editServer: '编辑 MCP 服务器',
      serverName: '服务器名称',
      serverNamePlaceholder: '例如: my-mcp-server',
      serverType: '类型',
      command: '启动命令',
      envVars: '环境变量',
      envKey: '变量名',
      envValue: '值',
      enabled: '启用',
      install: '安装',
      requiresEnv: '需要环境变量',
      confirmDelete: '确定要删除 {name} 吗？',
      tools: '工具',
      noTools: '暂无可用工具',
      viewDocs: '查看文档',
      configTips: '配置说明',
    },
    plugins: {
      title: '插件',
      installed: '已安装',
      notInstalled: '未安装',
      uninstall: '卸载',
      ohMyOpenCodeDesc: '强大的 OpenCode 增强插件，提供多 Agent 协作、后台任务等功能',
      sisyphusDesc: '智能任务编排 Agent',
      multiAgent: '多 Agent 协作（Oracle、Librarian、Explorer 等）',
      ultrawork: 'Ultrawork 模式 - 输入 ulw 自动完成复杂任务',
      claudeCompat: 'Claude Code 兼容层',
      antigravityDesc: '通过 Google OAuth 认证访问 Gemini 和 Claude 模型',
      kiroDesc: '通过 AWS Kiro 认证访问 Claude 4.5 模型，提供 550+ 免费请求',
      uiuxDesc: 'AI 驱动的 UI/UX 设计系统生成器，提供专业设计智能',
      googleOAuth: '使用 Google 账号登录认证',
      geminiModels: '访问 Gemini 3 Pro/Flash 等模型',
      claudeModels: '访问 Claude Opus/Sonnet 4.5 Thinking 模型',
      multiAccount: '支持多账号负载均衡',
      uiuxStyles: '57种UI样式 + 95个调色板',
      uiuxSystem: '智能设计系统生成',
      uiuxPlatforms: '多平台支持 (React/Vue/Tailwind)',
      uiuxRules: '100个行业特定规则',
      tipTitle: '使用提示',
      tipText: '安装后在聊天中输入 "ultrawork" 或 "ulw" 即可激活增强模式，Agent 会自动规划和执行复杂任务。',
      authTipTitle: '认证提示',
      authTipText: '安装后点击"认证"按钮，在终端菜单中选择 Google 完成账号认证。',
      kiroAuthTipTitle: 'Kiro 认证提示',
      kiroAuthTipText: '安装后点击"认证"按钮，在终端中选择 "Other"，然后输入 "kiro" 并按回车。如果认证失败，请：1) 确保有 AWS Builder ID 账号，2) 检查网络连接，3) 手动打开浏览器访问显示的认证链接，4) 重启 OpenCode 后重试。',
      uiuxTipTitle: '使用说明',
      uiuxTipText: '安装后可在聊天中使用，例如："帮我设计一个现代化的登录页面"、"创建一套 SaaS 产品的设计系统"',
      authenticate: '认证',
      authenticating: '认证中...',
      update: '升级',
      updating: '升级中...',
      manualAuth: '手动认证',
      troubleshoot: '故障排除',
      restartOpenCode: '重启 OpenCode',
      restarting: '正在重启...',
      restartHint: '修改插件配置后需要重启 OpenCode 才能生效',
      fix: '修复',
      fixHint: '禁用 Google 认证，避免与 Antigravity Auth 冲突',
    },
  },
  maven: {
    clean: '清理',
    compile: '编译',
    test: '测试',
    package: '打包',
    install: '安装',
    cleanInstall: '清理并安装',
    springRun: '运行 Spring Boot',
    tree: '依赖树',
  },
  search: {
    placeholder: '搜索文件内容...',
    replacePlaceholder: '替换为...',
    results: '个结果',
    noResults: '未找到结果',
    caseSensitive: '区分大小写',
    useRegex: '使用正则表达式',
    toggleReplace: '切换替换',
    replaceConfirm: '确定要将 "{search}" 替换为 "{replace}" 吗？',
    replaceSuccess: '成功替换 {count} 个文件',
    replaceFailed: '替换失败',
  },
  git: {
    noFolder: '未打开文件夹',
    noRepo: '当前目录不是 Git 仓库',
    changes: '更改',
    noChanges: '没有更改',
    stage: '暂存',
    discard: '丢弃',
    commit: '提交',
    push: '推送',
    pull: '拉取',
    refresh: '刷新',
    commitMessage: '请输入提交信息',
    commitFailed: '提交失败',
    pushFailed: '推送失败',
    pullFailed: '拉取失败',
    discardConfirm: '确定要丢弃此文件的更改吗？',
  },
}
