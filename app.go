...
func (a *App) GetMCPStatus() (map[string]MCPServerStatus, error) {
	...
	var lastStatus map[string]MCPServerStatus
...
func (a *App) GetMCPToolsPrompt() (string, error) {
	...
	for _, name := range enabledServers {
		switch name {
		case "mysql":
			prompt += fmt.Sprintf("- mcp_%s_execute_sql: Execute SQL query on MySQL database (query: string)\n", name)
		case "filesystem":
			prompt += fmt.Sprintf("- mcp_%s_read_file: Read file contents (path: string)\n", name)
			prompt += fmt.Sprintf("- mcp_%s_write_file: Write to file (path: string, content: string)\n", name)
			prompt += fmt.Sprintf("- mcp_%s_list_directory: List directory (path: string)\n", name)
		case "redis":
			prompt += fmt.Sprintf("- mcp_%s_redis_command: Execute Redis command (command: string)\n", name)
		case "postgres":
			prompt += fmt.Sprintf("- mcp_%s_query: Execute PostgreSQL query (query: string)\n", name)
		default:
			prompt += fmt.Sprintf("- mcp_%s: MCP server available\n", name)
		}
	}
	prompt += "Use these tools when the user asks about database queries or file operations.\n"
